...
internals.authenticate = function (request, reply) {
    ...
    Wreck.get(internals.options.profileUrl + encodeURIComponent(accessToken), internals.options.requestOptions, function(err, res, payload) {
        ...
        internals.options.validateFunc(payload, accessToken, function(err, valid, credentials) { // bring your own checks
            if (err) {
              return reply(Boom.wrap(err));
            }
            else if (!valid) {
              return reply(Boom.unauthorized('Invalid credentials'), null, { credentials: credentials });
            }
            else {
              return reply.continue({ credentials: credentials, artifacts: accessToken });
            }
        });
    });
};
